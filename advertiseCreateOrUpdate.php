...
if ($id) {
    $stmt = $conn->prepare("SELECT image FROM advertise WHERE id=?");
    ...
    $sql = "UPDATE advertise SET 
        title=?, description=?, image=?, status=?, updated_at=NOW() 
        WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssii", $title, $description, $finalImage, $status, $id);
    ...
    $sql = "INSERT INTO advertise 
        (title, description, image, status, created_at) 
        VALUES (?, ?, ?, ?, NOW())";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssi", $title, $description, $finalImage, $status);
